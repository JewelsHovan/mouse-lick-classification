...
def update_sample_weights(y_train, predictions_train, learning_rate):
    """
    Simplified sample weight updates using the sigmoid function.
    """
    x = learning_rate * y_train * predictions_train
    weights = 1 / (1 + np.exp(x)).astype(np.float32)
    weights /= np.sum(weights)
    return weights

def compute_loss(y, F):
    """
    Simplified loss computation using the sigmoid function.
    """
    x = y * F
    losses = 1 / (1 + np.exp(x))
...
class GentleBoost(BaseEstimator, ClassifierMixin):
    ...
    def _fit_single_estimator(self, m, X_train, y_train, sample_weights, X_val=None, y_val=None):
        """
        Fit a single estimator in the boosting sequence.
        """
    ...
    def fit(self, X, y):
        """
        Modified fit method with consistent data types and precomputed learning rates.
        """
        # Convert inputs to memory-efficient data types
        ...
        # Convert y to -1 and 1 for boosting
        ...
        for m in range(self.n_estimators):
            ...
            self.train_scores.append(loss_train)

            if X_val is not None:
                F_val += current_lr * predictions_val
                loss_val = compute_loss(y_val, F_val)
                self.val_scores.append(loss_val)
    ...
    def predict(self, X):
        """
        Optimized prediction with precomputed learning rates.
        """
        X = np.asarray(X, dtype=np.float32)
    ...
    def decision_function(self, X):
        """
        Optimized decision function with precomputed learning rates.
        """
        X = np.asarray(X, dtype=np.float32)
        F = np.zeros(X.shape[0], dtype=np.float32)
...
if __name__ == "__main__":
    ...
    results = test_gentleboost(**model_params)
